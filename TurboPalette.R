...
write.table(x = turbo_colormap_data_HEX,file = "C:/Users/dbara/Documents/Blog/turbo_hex.txt",quote = F,row.names = F,col.names = F)
...
################# Examples from https://davidbaranger.com/2019/08/22/using-the-new-turbo-palette-from-google-in-r/
...
my_comp_col  <- get_scale_color(TurboPalette(256))

# Scatterplot
gg <- ggplot(midwest, aes(x=area, y=poptotal)) + 
  geom_point(aes(col=state, size=popdensity)) + 
 scale_colour_manual(values = Turbo(pal.min = 0.5,pal.max = .95,out.colors = 5) )+
...
ggplot(df, aes(monthweek, weekdayf, fill = VIX.Close)) + 
  ...
  scale_fill_gradientn(colours = Turbo(pal.min = .6,pal.max = 1,reverse = T)) + ## this is new
